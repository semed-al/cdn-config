...
SELECT
    ...
    area_conhecimento.agrupar_descritores,
    (case when area_conhecimento.agrupar_descritores = true 
        then string_agg(distinct area_conhecimento.nome, ', ')
        else string_agg(distinct view_componente_curricular.nome, ', ')
        end) AS disciplina
...
INNER JOIN modules.area_conhecimento ON TRUE
    AND area_conhecimento.id = view_componente_curricular.area_conhecimento_id
