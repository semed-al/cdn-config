...
class AgeDistortionInSerieReport extends Portabilis_Report_ReportCore
{
    ...
    {
        return $this->args['modelo'] == 'regular' ? 'age-distortion-in-serie' : 'age-distortion-in-serie-not-regular';
    }
    ...
    public function getJsonData()
    {
		...
		if ($this->args['modelo'] == 'regular'){
			...
			$data = [
				'header' => Portabilis_Utils_Database::fetchPreparedQuery($queryHeaderReport),
				'main' => Portabilis_Utils_Database::fetchPreparedQuery($this->getSqlDatasetInSerieNotRegularReport()),
			];
		}
    ...
    public function getSqlMainReport()
    {
		...
		$data_inicial_query = ' AND TRUE ';
        $data_final_query = ' AND TRUE ';

        if ($this->args['data_inicial']) {
            $data_inicial_query = " AND m.data_matricula >= '{$data_inicial}'::date ";
        }

        if ($this->args['data_final']) {
            $data_final_query = " AND m.data_matricula <= '{$data_final}'::date ";
        }
...
(select cast(count(m.cod_matricula) as float) as num_alunos
	...
	public function getSqlDatasetInSerieNotRegularReport()
    {
        $situacao = $this->args['situacao'] ?: 0;
        $escola = $this->args['escola'] ?: 0;
        $serie = $this->args['serie'] ?: 0;
